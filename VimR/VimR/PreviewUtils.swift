...
class PreviewUtils {
  ...
  static func state(
    for status: PreviewState.Status,
    baseUrl: URL,
  ...
  private static func serverUrl(for uuid: UUID, baseUrl: URL, lastComponent: String) -> URL {
    baseUrl.appendingPathComponent("\(uuid)/\(markdownPath)/\(lastComponent)")
  }

  private static func htmlUrl(with uuid: UUID) -> URL {
    self.tempDir.appendingPathComponent("\(uuid)-markdown-index.html")
  }
